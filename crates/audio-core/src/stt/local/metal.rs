...
//!
//! Cross-platform facade: `open_whisper_backend()` is always defined; macOS runs
//! whisper-rs + Metal, other targets return `None`.
...
mod imp {
    ...
    impl WhisperBackend for MetalWhisperEngine {
        fn transcribe(&self, samples: &[f32], language: &str) -> Result<TranscribeOutcome> {
            let avg_rms = super::super::common::rms_energy(samples);
            ...
            params.set_language(Some(super::super::common::whisper_language(language)));
    ...
    fn resolve_ggml_model_path() -> PathBuf {
    ...
    /// `Some(Ok)` / `Some(Err)` on macOS; never `None` on this platform.
    pub fn open_whisper_backend() -> Option<Result<Arc<dyn WhisperBackend>>> {
        Some(MetalWhisperEngine::new().map(|engine| Arc::new(engine) as Arc<dyn WhisperBackend>))
    }
}
...
mod imp {
    ...
    use super::super::common::WhisperBackend;
...
pub use imp::open_whisper_backend;
