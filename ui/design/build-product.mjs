#!/usr/bin/env bun
/**
 * Phase 8 — rebuild Foundations … Product-complete screens from tokens.json.
 * Usage: bun ui/design/build-product.mjs
 */
...
/** 16×16 toolbar icon slot (stroke frame + simple glyph). */
function iconSlot(name, x, y, pal, glyph = 'dot') {
  const s = 16;
  const slot = box(name, x, y, s, s, pal['bg/base'], 3);
  stroke(slot, pal['text/muted']);
  if (glyph === 'mic') {
    box(name + ' / mic', x + 5, y + 3, 6, 9, pal['text/muted'], 3);
    box(name + ' / base', x + 4, y + 12, 8, 2, pal['text/muted'], 1);
  } else if (glyph === 'monitor') {
    const body = box(name + ' / body', x + 2, y + 3, 12, 8, pal['bg/base'], 1);
    stroke(body, pal['text/muted']);
    box(name + ' / stand', x + 6, y + 11, 4, 2, pal['text/muted'], 0);
  } else if (glyph === 'stop') {
    box(name + ' / sq', x + 4, y + 4, 8, 8, pal['text/muted'], 1);
  } else if (glyph === 'clock') {
    const ring = box(name + ' / ring', x + 2, y + 2, 12, 12, pal['bg/base'], 6);
    stroke(ring, pal['text/muted']);
    box(name + ' / hand', x + 8, y + 5, 1, 5, pal['text/muted'], 0);
  } else if (glyph === 'export') {
    box(name + ' / arr', x + 7, y + 3, 2, 7, pal['text/muted'], 0);
    box(name + ' / bar', x + 4, y + 11, 8, 2, pal['text/muted'], 0);
  } else if (glyph === 'gear') {
    const c = box(name + ' / c', x + 5, y + 5, 6, 6, pal['bg/base'], 3);
    stroke(c, pal['text/muted']);
  } else if (glyph === 'lines') {
    box(name + ' / l1', x + 3, y + 4, 10, 2, pal['text/muted'], 1);
    box(name + ' / l2', x + 3, y + 7, 7, 2, pal['text/muted'], 1);
    box(name + ' / l3', x + 3, y + 10, 9, 2, pal['text/muted'], 1);
  } else if (glyph === 'bookmark') {
    const bm = box(name + ' / bm', x + 4, y + 3, 8, 11, pal['bg/base'], 1);
    stroke(bm, pal['text/muted']);
  } else if (glyph === 'trash') {
    box(name + ' / lid', x + 4, y + 4, 8, 2, pal['text/muted'], 0);
    const bin = box(name + ' / bin', x + 5, y + 6, 6, 7, pal['bg/base'], 1);
    stroke(bin, pal['text/muted']);
  }
  return slot;
}

/** Dock control with icon (+ optional label). */
function dockBtn(name, x, y, w, h, pal, label, glyph, filled, accentHex) {
  const bg = filled ? (accentHex || pal['semantic/live']) : pal['bg/base'];
  const b = box(name, x, y, w, h, bg, TOKENS.radius.sm);
  if (!filled) stroke(b, pal['border/subtle']);
  iconSlot(name + ' / icon', x + Math.round((w - 16) / 2), label ? y + 5 : y + Math.round((h - 16) / 2), pal, glyph);
  if (label) lbl(name + ' / label', x, y + h - 19, w, 16, label, 11, filled ? pal['bg/elevated'] : pal['text/primary'], 'Medium');
  return b;
}
...
y = section('Icon slots (toolbar 16px)', y);
cy = y;
const iconDefs = [
  ['mic-out', 'Mic Out', 'mic'], ['mic-in', 'Mic In', 'mic'], ['monitor', 'Monitor', 'monitor'],
  ['logs', 'Logs', 'lines'], ['tab-audio', 'Tab Audio', 'monitor'], ['compact', 'Compact', 'lines'],
  ['bookmark', 'Saved', 'bookmark'], ['history', 'History', 'clock'], ['export', 'Export', 'export'],
  ['clear', 'Clear', 'trash'], ['settings', 'Settings', 'gear'],
];
let ix = m;
for (let i = 0; i < iconDefs.length; i++) {
  const [id, label, glyph] = iconDefs[i];
  const cell = box('component / icon / ' + id, ix, cy, 72, 56, L['bg/elevated'], TOKENS.radius.sm);
  stroke(cell, L['border/subtle']);
  iconSlot('component / icon / ' + id + ' / slot', ix + 28, cy + 8, L, glyph);
  lbl('component / icon / ' + id + ' / label', ix, cy + 30, 72, 22, label, 10, L['text/muted'], 'Regular', 'CENTER');
  ix += 84;
  if (i === 5) { ix = m; cy += 72; }
}
y = cy + 72;
mountPage('Screens', 1280, 6800, L['bg/base']);
y = 48;
txt('title', 'Screens', 48, y, 32, L['text/primary'], 'Bold');
txt('subtitle', 'Product-complete — Granola calm + real feature inventory', 48, y + 42, 14, L['text/secondary']);
...
function callScreen(prefix, oy, pal) {
  ...
  iconSlot(prefix + ' / settings / icon', SW - m - 36, tbY + 14, pal, 'gear');
  ...
  box(prefix + ' / dock / stop', dockX + 12, dockY + 10, 88, 40, pal['semantic/danger'], TOKENS.radius.md);
  iconSlot(prefix + ' / dock / stop / icon', dockX + 28, dockY + 16, pal, 'stop');
  lbl(prefix + ' / dock / stop / label', dockX + 12, dockY + 10, 88, 40, 'Stop', 13, pal['bg/elevated'], 'Semi Bold');
  dockBtn(prefix + ' / dock / mic-out', dockX + 112, dockY + 10, 44, 40, pal, null, 'mic', true);
  dockBtn(prefix + ' / dock / mic-in', dockX + 164, dockY + 10, 44, 40, pal, null, 'mic', false);
  dockBtn(prefix + ' / dock / monitor', dockX + 220, dockY + 10, 108, 40, pal, 'Monitor', 'monitor', false);
  dockBtn(prefix + ' / dock / more', dockX + dockW - 52, dockY + 10, 40, 40, pal, null, 'lines', false);
...
function callOverflowScreen(prefix, oy, pal) {
  const end = callScreen(prefix, oy, pal);
  const SW = 1280;
  const dockW = 720;
  const dockX = Math.round((SW - dockW) / 2);
  const dockY = oy + 840 - 108;
  const menuX = dockX + dockW - 200;
  const menuY = dockY - 168;
  const menu = box(prefix + ' / overflow', menuX, menuY, 184, 160, pal['bg/elevated'], TOKENS.radius.md);
  stroke(menu, pal['border/subtle']);
  const items = [['Logs', 'lines'], ['Tab Audio', 'monitor'], ['Compact', 'lines'], ['Saved', 'bookmark'], ['Export', 'export'], ['Clear', 'trash']];
  let my = menuY + 8;
  for (let i = 0; i < items.length; i++) {
    const row = box(prefix + ' / overflow / ' + i, menuX + 8, my, 168, 24, pal['bg/base'], TOKENS.radius.sm);
    iconSlot(prefix + ' / overflow / ' + i + ' / icon', menuX + 14, my + 4, pal, items[i][1]);
    lbl(prefix + ' / overflow / ' + i + ' / label', menuX + 36, my, 132, 24, items[i][0], 12, pal['text/primary'], 'Medium', 'LEFT');
    my += 26;
  }
  return end;
}

function settingsAccordionRow(prefix, panelX, y, panelW, title, expanded, pal, fields) {
  const headH = 40;
  const head = box(prefix + ' / head', panelX + 12, y, panelW - 24, headH, pal['bg/base'], TOKENS.radius.sm);
  stroke(head, pal['border/subtle']);
  lbl(prefix + ' / title', panelX + 24, y, panelW - 72, headH, title, 13, pal['text/primary'], 'Semi Bold', 'LEFT');
  lbl(prefix + ' / chev', panelX + panelW - 44, y, 24, headH, expanded ? '▾' : '▸', 14, pal['text/muted'], 'Regular', 'CENTER');
  let nextY = y + headH + 4;
  if (expanded && fields) {
    const bodyH = fields.length * 68 + 24;
    const body = box(prefix + ' / body', panelX + 12, nextY, panelW - 24, bodyH, pal['bg/base'], TOKENS.radius.sm);
    stroke(body, pal['border/subtle']);
    let fy = nextY + 12;
    for (let i = 0; i < fields.length; i++) {
      lbl(prefix + ' / field / ' + i + ' / label', panelX + 24, fy, 120, 28, fields[i][0], 11, pal['text/muted'], 'Regular', 'LEFT');
      const inp = box(prefix + ' / field / ' + i + ' / input', panelX + 24, fy + 28, panelW - 48, 32, pal['bg/elevated'], TOKENS.radius.sm);
      stroke(inp, pal['border/subtle']);
      lbl(prefix + ' / field / ' + i + ' / value', panelX + 24, fy + 28, panelW - 48, 32, fields[i][1], 12, pal['text/primary'], 'Regular', 'LEFT');
      fy += 68;
    }
    nextY += bodyH + 8;
  } else {
...
function settingsFullScreen(prefix, oy, pal) {
  const SW = 1280;
  const SH = 880;
  box(prefix + ' / bounds', 0, oy, SW, SH, pal['bg/base']);
  txt('screen / tag / ' + prefix, 'Settings / Full (accordion)', m, oy - 24, 12, L['text/muted'], 'Semi Bold');
  box(prefix + ' / dim', 0, oy, SW, SH, pal['border/subtle']);
  const panelW = 480;
  const panelX = SW - panelW;
  box(prefix + ' / panel', panelX, oy, panelW, SH, pal['bg/elevated']);
  stroke(box(prefix + ' / panel-stroke', panelX, oy, panelW, SH, pal['bg/elevated']), pal['border/subtle']);
  lbl(prefix + ' / title', panelX, oy + 12, panelW, 36, 'Settings', 18, pal['text/primary'], 'Bold');
  lbl(prefix + ' / close', panelX + panelW - 44, oy + 12, 32, 36, '×', 20, pal['text/muted'], 'Regular', 'CENTER');
  let sy = oy + 56;
  sy = settingsAccordionRow(prefix + ' / sec-keys', panelX, sy, panelW, 'API Keys', true, pal, [
    ['STT Engine', 'Local — Whisper (Metal / CPU)'],
    ['Translation', 'Local — Opus-MT (ru↔en)'],
    ['Whisper model', 'Base Q8_0 — best Metal quality'],
  ]);
  sy = settingsAccordionRow(prefix + ' / sec-lang', panelX, sy, panelW, 'Languages', false, pal, null);
  sy = settingsAccordionRow(prefix + ' / sec-voice', panelX, sy, panelW, 'Voice', false, pal, null);
  sy = settingsAccordionRow(prefix + ' / sec-audio', panelX, sy, panelW, 'Audio Devices', false, pal, null);
  sy = settingsAccordionRow(prefix + ' / sec-advanced', panelX, sy, panelW, 'Advanced', false, pal, null);
  const footY = oy + SH - 64;
  box(prefix + ' / footer', panelX, footY, panelW, 64, pal['bg/elevated']);
  box(prefix + ' / save', panelX + 16, footY + 12, panelW - 32, 40, pal['accent/calm'], TOKENS.radius.md);
  lbl(prefix + ' / save / label', panelX + 16, footY + 12, panelW - 32, 40, 'Save & Restart Engine', 14, pal['bg/elevated'], 'Semi Bold');
  return oy + SH + 48;
}

function historyListScreen(prefix, oy, pal) {
  const SW = 1280;
  const SH = 720;
  box(prefix + ' / bounds', 0, oy, SW, SH, pal['bg/base']);
  txt('screen / tag / ' + prefix, 'History / List', m, oy - 24, 12, L['text/muted'], 'Semi Bold');
  box(prefix + ' / top', 0, oy, SW, 52, pal['bg/elevated']);
  box(prefix + ' / top / rule', 0, oy + 51, SW, 1, pal['border/subtle']);
  lbl(prefix + ' / top / title', m, oy, 200, 52, 'Call History', 16, pal['text/primary'], 'Bold', 'LEFT');
  box(prefix + ' / top / back', SW - m - 120, oy + 10, 88, 32, pal['bg/base'], TOKENS.radius.sm);
  stroke(box(prefix + ' / top / back2', SW - m - 120, oy + 10, 88, 32, pal['bg/base'], TOKENS.radius.sm), pal['border/subtle']);
  lbl(prefix + ' / top / back / label', SW - m - 120, oy + 10, 88, 32, '← Live', 12, pal['text/primary'], 'Medium');
  const colX = Math.round((SW - 720) / 2);
  let cy = oy + 72;
  const cards = [
    ['Today, 14:32', 'EN ↔ RU', '12 min · 34 phrases', 'Discussed project timeline and document handoff.'],
    ['Yesterday, 09:15', 'EN ↔ RU', '8 min · 21 phrases', 'Quick sync on release checklist.'],
    ['Mon, 18:40', 'EN ↔ DE', '22 min · 56 phrases', 'Client demo with live translation.'],
  ];
  for (let i = 0; i < cards.length; i++) {
    const c = box(prefix + ' / card / ' + i, colX, cy, 720, 96, pal['bg/elevated'], TOKENS.radius.lg);
    stroke(c, pal['border/subtle']);
    lbl(prefix + ' / card / ' + i + ' / date', colX + 16, cy + 12, 200, 24, cards[i][0], 14, pal['text/primary'], 'Semi Bold', 'LEFT');
    box(prefix + ' / card / ' + i + ' / lang', colX + 600, cy + 12, 96, 24, pal['bg/base'], TOKENS.radius.pill);
    stroke(box(prefix + ' / card / ' + i + ' / lang2', colX + 600, cy + 12, 96, 24, pal['bg/base'], TOKENS.radius.pill), pal['border/subtle']);
    lbl(prefix + ' / card / ' + i + ' / lang / t', colX + 600, cy + 12, 96, 24, cards[i][1], 11, pal['text/secondary'], 'Medium');
    lbl(prefix + ' / card / ' + i + ' / meta', colX + 16, cy + 40, 400, 20, cards[i][2], 12, pal['text/muted'], 'Regular', 'LEFT');
    lbl(prefix + ' / card / ' + i + ' / sum', colX + 16, cy + 62, 688, 24, cards[i][3], 13, pal['text/secondary'], 'Regular', 'LEFT');
    cy += 108;
  }
  return oy + SH + 48;
}

function historyDetailScreen(prefix, oy, pal) {
  const SW = 1280;
  const SH = 840;
  box(prefix + ' / bounds', 0, oy, SW, SH, pal['bg/base']);
  txt('screen / tag / ' + prefix, 'History / Detail', m, oy - 24, 12, L['text/muted'], 'Semi Bold');
  box(prefix + ' / top', 0, oy, SW, 52, pal['bg/elevated']);
  lbl(prefix + ' / top / title', m, oy, 240, 52, 'Today, 14:32', 16, pal['text/primary'], 'Bold', 'LEFT');
  box(prefix + ' / top / back', m, oy + 10, 72, 32, pal['bg/base'], TOKENS.radius.sm);
  stroke(box(prefix + ' / top / back2', m, oy + 10, 72, 32, pal['bg/base'], TOKENS.radius.sm), pal['border/subtle']);
  lbl(prefix + ' / top / back', m, oy + 10, 72, 32, '← Back', 12, pal['text/primary'], 'Medium');
  box(prefix + ' / top / summary', m + 84, oy + 10, 96, 32, pal['accent/calm'], TOKENS.radius.sm);
  lbl(prefix + ' / top / summary / l', m + 84, oy + 10, 96, 32, '★ Summary', 12, pal['bg/elevated'], 'Medium');
  box(prefix + ' / top / delete', SW - m - 88, oy + 10, 88, 32, pal['bg/base'], TOKENS.radius.sm);
  stroke(box(prefix + ' / top / delete2', SW - m - 88, oy + 10, 88, 32, pal['bg/base'], TOKENS.radius.sm), pal['semantic/danger']);
  lbl(prefix + ' / top / delete / l', SW - m - 88, oy + 10, 88, 32, '✕ Delete', 12, pal['semantic/danger'], 'Medium');
  const colX = Math.round((SW - 720) / 2);
  const sum = box(prefix + ' / summary', colX, oy + 68, 720, 72, pal['bg/elevated'], TOKENS.radius.md);
  stroke(sum, pal['border/subtle']);
  txt(prefix + ' / summary / text', 'Team agreed to send the document after the call. Release checklist reviewed; no blockers.', colX + 16, oy + 84, 13, pal['text/secondary'], 'Regular', 688);
  let uy = oy + 156;
  const utts = [
    ['Can you hear me clearly?', 'Меня хорошо слышно?', true],
    ['Yes, perfectly fine.', 'Да, отлично слышно.', false],
    ["I'll send the document after the call.", 'Отправлю документ после звонка.', true],
  ];
  for (let i = 0; i < utts.length; i++) {
    const w = 400;
    const x = utts[i][2] ? colX + 720 - w : colX;
    const bg = utts[i][2] ? pal['bg/bubble-out'] : pal['bg/bubble-in'];
    const b = box(prefix + ' / utt / ' + i, x, uy, w, 56, bg, TOKENS.radius.md);
    stroke(b, pal['border/subtle']);
    lbl(prefix + ' / utt / ' + i + ' / orig', x + 12, uy + 8, w - 24, 18, utts[i][0], 12, pal['text/muted'], 'Regular', 'LEFT');
    lbl(prefix + ' / utt / ' + i + ' / trans', x + 12, uy + 28, w - 24, 20, utts[i][1], 14, pal['text/primary'], 'Medium', 'LEFT');
    uy += 68;
  }
  return oy + SH + 48;
}
...
screenY = callOverflowScreen('screen / call-overflow', screenY, L);
screenY = emptyScreen('screen / empty', screenY, L);
screenY = settingsFullScreen('screen / settings-full', screenY, L);
screenY = historyListScreen('screen / history-list', screenY, L);
historyDetailScreen('screen / history-detail', screenY, L);
...
console.log('Foundations + Tokens + Elements + Patterns + Components + Product screens rebuilt from tokens.json');
